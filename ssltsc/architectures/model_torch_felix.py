...
class IMU_m(nn.Module):

    def __init__(self, dropout=0.2, kernel=4, filters=200, td_layer="BILSTM", num_classes=15, 
                 length_ts=64, imu_vals=13):
        super(IMU_m, self).__init__()
        self.td_layer = td_layer
        print('build IMU with ' + self.td_layer + '...')
        self.feature_extraction = nn.Sequential(
            nn.Conv1d(in_channels=imu_vals, out_channels=filters, kernel_size=kernel, padding='same'),
            nn.ReLU(),
        ...
        # if self.td_layer == "TCN":
        #     self.model = TemporalConvNet(num_inputs=filters,
        #                                  num_channels=[100],
        #                                  kernel_size=3,
        #                                  dropout=0.1,
        #                                  use_weight_norm = False)
        #     # self.model_act = nn.ReLU()
        #     # self.model_act = nn.LeakyReLU()
        #     self.model_act = nn.ReLU6()
        #     out_features_model = int(100 * ((length_ts / 2) / 2))
        #     # td = TCN(nb_filters=100, dropout_rate=0.1, activation=tf.nn.relu6, return_sequences=True)(drop2)
...
if __name__ == "__main__":
    model = IMU_m(0.2, 4, 200, td_layer="BILSTM", num_classes=60, imu_vals=13)
    print(model)